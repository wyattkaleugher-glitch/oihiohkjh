...
public class TechniqueCommands implements TabExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        if (args.length == 0) {
            sender.sendMessage("§cUsage: /jjk <ability1|ability2|domain|setclass|info|withdraw|setgrade|giveitem|givece|setmaxce|reload>");
            return true;
        }
        ...
        // 👑 ADMIN COMMAND: ASSIGN JUJUTSU GRADE
        if (action.equals("setgrade")) {
            ...
            // Combine trailing array strings to format clean grade names containing spacing parameters
            StringBuilder gradeBuilder = new StringBuilder();
            for (int i = 2; i < args.length; i++) {
                gradeBuilder.append(args[i]).append(" ");
            }
            ...
            profileManager.saveProfile(target.getUniqueId()); // Push values safely into local storage data layout
        ...
        // 📜 PLAYER COMMAND: VIEW STAT PROFILE CARD INFO
        ...
        // 💎 PLAYER COMMAND: WITHDRAW PHYSICAL TOKENS
        if (action.equals("withdraw")) {
            if (args.length < 2) {
                player.sendMessage("§cSpecify how much energy to withdraw! Usage: /jjk withdraw <amount>");
                return true;
            }
            try {
                int amountToWithdraw = Integer.parseInt(args[1]);
                if (amountToWithdraw <= 0) {
                    player.sendMessage("§cWithdrawal parameters must be a positive non-zero value!");
                    return true;
                }
                if (profile.getCursedEnergy() < amountToWithdraw) {
                    player.sendMessage("§cInsufficent reserves! You only have §b" + profile.getCursedEnergy() + " CE §cavailable in storage.");
                    return true;
                }

                // Check if player inventory array stack has space for the physical amethyst asset
                if (player.getInventory().firstEmpty() == -1) {
                    player.sendMessage("§cYour inventory inventory slots are completely full!");
                    return true;
                }

                // Deduct from profile memory cache balance safely
                profile.setCursedEnergy(profile.getCursedEnergy() - amountToWithdraw);
                profileManager.saveProfile(player.getUniqueId());

                // Generate and inject physical item crystal token
                ItemStack crystal = new ItemStack(Material.AMETHYST_SHARD);
                ItemMeta meta = crystal.getItemMeta();
                if (meta != null) {
                    meta.setDisplayName("§d§lCrystallized Cursed Energy");
                ...
                player.sendMessage("§aSuccessfully withdrew §e" + amountToWithdraw + " CE §ainto a physical Crystal Shard!");
                player.playSound(player.getLocation(), Sound.ENTITY_ITEM_PICKUP, 1.0F, 1.0F);

            } catch (NumberFormatException e) {
                player.sendMessage("§cWithdrawal amount parameter must be a structural whole number value!");
            }
            return true;
        }
        ...
        switch (action) {
            ...
            case "domain" -> {
                ...
                if (profile.getCursedEnergy() < domainCost) {
                    player.sendMessage("§cInsufficent energy to form a domain! (Requires " + domainCost + " CE)");
                    return true;
                }
        ...
        // Save adjustments back down safely to disk
        profileManager.saveProfile(player.getUniqueId());
        return true;
    }
    ...
    public @Nullable List<String> onTabComplete(@NotNull CommandSender sender, @NotNull Command command, @NotNull String alias, @NotNull String[] args) {
        ...
        if (args.length == 1) {
            ...
            if (sender.hasPermission("jjk.admin")) subs.addAll(Arrays.asList("reload", "givece", "setmaxce", "giveitem", "setgrade"));
        ...
        if (args.length == 2) {
            ...
            } else if (first.equals("givece") || first.equals("setmaxce") || first.equals("giveitem") || first.equals("setgrade")) {
                if (sender.hasPermission("jjk.admin")) {
