...
public class TechniqueCommands implements TabExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        if (!(sender instanceof Player player)) {
            sender.sendMessage("§cOnly players can execute JJK commands.");
            return true;
        }
        ...
        switch (action) {
            ...
            case "clearcooldown" -> {
                ...
                targetProf.clearAllCooldowns();
                player.sendMessage("§aReset all cooldowns for " + target.getName());
                target.sendMessage("§b§lFLASH! §7All your ability cooldowns have been reset.");
            }
    ...
    private void handleWithdraw(Player player, PlayerProfile profile, String[] args) {
        ...
        if (type.equals("ce")) {
            if (args.length < 3) { player.sendMessage("§cUsage: /jjk withdraw ce <amount>"); return; }
            try {
    ...
    private ItemStack createSpear() {
        ...
        m.setLore(List.of("§7Pierces and nullifies all techniques.", "§fLeft-click Domain Walls to shatter them."));
        item.setItemMeta(m);
        return item;
    }
    ...
    private void sendHelp(Player p) {
        ...
        p.sendMessage("§e/jjk withdraw <ce|class> [amt] §7- Store power in items");
        if (p.hasPermission("jjk.admin")) {
            p.sendMessage("§c/jjk clearcooldown [player] §7- Reset timers");
            p.sendMessage("§c/jjk setgrade <player> <grade>");
            p.sendMessage("§c/jjk giveclass <player> <type>");
            p.sendMessage("§c/jjk givespear §7- Grant the ISOH");
        }
    }
    ...
    public @Nullable List<String> onTabComplete(@NotNull CommandSender sender, @NotNull Command command, @NotNull String alias, @NotNull String[] args) {
        if (args.length == 1) return Arrays.asList("info", "withdraw", "clearcooldown", "setgrade", "giveclass", "givespear");
