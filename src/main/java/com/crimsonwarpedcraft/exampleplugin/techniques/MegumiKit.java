...
public class MegumiKit {
    public static void execute(Player p, PlayerProfile prof, int slot) {
        if (slot == 1) { 
            ...
            // Apply the Mahoraga "General" buffs to the player
            p.addPotionEffect(new PotionEffect(PotionEffectType.STRENGTH, 600, 3));
            p.addPotionEffect(new PotionEffect(PotionEffectType.RESISTANCE, 600, 2));
